...
fruitObj.prototype.draw = function () {
    for (var i = 0 ; i < this.num ; i++){
        if (this.alive[i]) {
            ...
            ctx2.drawImage(pic,this.x[i] - this.scale[i] * 0.5,this.y[i] - this.scale[i] * 0.5,this.scale[i],this.scale[i]);    //绘制果实
            /*当果实的Y坐标小于10
             * 把果实的状态变为false
...
fruitObj.prototype.born = function (i) {
    var aneID = Math.floor(Math.random() * ane.num);                                                                                    //随即选择一个海葵的X坐标
    this.x[i] = ane.x[aneID];                                                                                                           //果实出生的X坐标
    this.y[i] = canvasHeight - ane.length[aneID];                                                                                       //出生的Y坐标
